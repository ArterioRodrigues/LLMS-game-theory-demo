export const fetchAllResponses = async (prompt: string, models: string[], callback: any) => {
  ...
  const promises = models.map(async model => {
    try {
      ...
      callback((prev: any) => ({
        ...prev,
        [model]: response.error ? response.error.message : (response.choices && response.choices[0]?.message?.content) || 'No response received',
      }));
...
const fetchGPT = async (prompt: string, model: string) => {
  try {
    ...
    return {
      error: { message: error instanceof Error ? error.message : 'Failed to fetch response' },
    };
  }
};
